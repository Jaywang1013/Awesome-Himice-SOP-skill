...
在读取预算表或修改审批表前，先检查 `${CLAUDE_SKILL_DIR}/references/department-defaults.local.md`；没有时提示用户从 `${CLAUDE_SKILL_DIR}/references/department-defaults.example.md` 复制并仅在本地补全。每次调用都必须先展示：
...
3. 从日程写活动起止、从预算写会议人数；可确认的活动内容与城市可填写。活动类型、业务类型、竞标状态、本地客户、客户来源/类型、联系人及比例按已确认的本地默认信息或本次修改填写；其余无法确认的字段保持空白或标 `[待确认]`。
